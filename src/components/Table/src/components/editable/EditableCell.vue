<template>
  <div class="editable-cell">
    <div v-show="!isEdit" class="editable-cell-content" @click="handleEdit">
      <span v-if="!column.editCellRender">{{ getValues }}</span>
      <Render v-else ref="Render" :value="getValues" />
...
<script lang="ts">
  ...
  export default defineComponent({
    name: 'EditableCell',
    components: { FormOutlined, CloseOutlined, CheckOutlined, CellComponent, Render },
    directives: {
      clickOutside,
    },
    props: {
    ...
    setup(props) {
      ...
      const getValues = computed(() => {
        const { editComponentProps, editValueMap, editCellRender } = props.column;

        const value = unref(currentValueRef);

        if (editCellRender && isFunction(editCellRender)) {
          return editCellRender(value);
        }
